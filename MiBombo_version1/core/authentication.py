...
Base de données PostgreSQL chiffrée avec gestion des rôles et permissions.
"""
...
class AuthenticationManager:
    ...
    def _generate_machine_key(self):
        ...
        try:
            ...
            machine_id = str(os.getuid()) + os.path.expanduser("~") + "MiBombo"
            
            # Dériver une clé avec PBKDF2
            kdf = PBKDF2HMAC(
                algorithm=hashes.SHA256(),
                length=32,
                salt=b"MiBomboAuthSalt",
                iterations=100000,
            )
    ...
    def _init_database(self):
        ...
        self.cursor.execute('''CREATE TABLE IF NOT EXISTS auth_users (
    ...
    def _create_default_admin(self):
        ...
        self.cursor.execute('SELECT id FROM auth_users WHERE username = ?', ("admin",))
        if self.cursor.fetchone() is None:
            ...
            ''', (
                ...
                self._encrypt("admin@mibombo.local"),
                json.dumps(["all"])
            ))
        ...
        try:
            ...
            if is_locked and locked_until:
                ...
                if datetime.now() < lock_time:
                    ...
                    # Débloquer le compte
                    self.cursor.execute('''
                        UPDATE auth_users SET is_locked = 0, failed_attempts = 0, locked_until = NULL
                        WHERE id = ?
                    ''', (user_id,))
                    self.connection.commit()
            ...
            if not self._verify_password(password, pwd_hash, salt):
                ...
                if failed_attempts >= self.max_login_attempts:
                    ...
                    self.cursor.execute('''
                        UPDATE auth_users SET failed_attempts = ?, is_locked = 1, locked_until = ?
                        WHERE id = ?
            ...
            self.cursor.execute('''
                UPDATE auth_users SET failed_attempts = 0, is_locked = 0, 
    ...
    def register_user(self, username: str, password: str, email: str, full_name: str = None) -> Tuple[bool, str]:
        """
        Enregistre un nouvel utilisateur (public).
        Le compte est créé inactif en attente de validation.
        """
        if len(password) < self.password_min_length:
            return False, f"Mot de passe trop court ({self.password_min_length} caractères min)"
        
        self._open()
        try:
            # Vérifier si l'utilisateur existe déjà
            self.cursor.execute('SELECT id FROM auth_users WHERE username = ?', (username,))
            ...
            # Créer l'utilisateur (inactif par défaut pour validation)
            self.cursor.execute('''
                INSERT INTO auth_users (username, password_hash, password_salt, role, 
                                   email, full_name, permissions, is_active, created_by)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                username,
                password_hash,
                salt,
                "viewer",  # Rôle par défaut
                self._encrypt(email) if email else None,
                self._encrypt(full_name) if full_name else None,
                json.dumps(ROLES["viewer"]["permissions"]),
                0,  # Inactif
                None # Pas de créateur (inscription publique)
            ))
            
            self.connection.commit()
            
            self._log_audit("user_registered", f"Inscription: {username}")
            
            return True, "Inscription réussie. Votre compte est en attente de validation."
    ...
    def update_user(self, user_id: int, **kwargs) -> Tuple[bool, str]:
        ...
        try:
            ...
            sql = f"UPDATE auth_users SET {', '.join(updates)} WHERE id = ?"
    ...
    def delete_user(self, user_id: int) -> Tuple[bool, str]:
        ...
        try:
            ...
            self.cursor.execute('''
                SELECT COUNT(*) FROM auth_users WHERE role = 'admin' AND is_active = 1 AND id != ?
            ''', (user_id,))
            
            admin_count = self.cursor.fetchone()[0]
            
            self.cursor.execute('SELECT role FROM auth_users WHERE id = ?', (user_id,))
            row = self.cursor.fetchone()
            if not row:
    ...
    def get_user_stats(self) -> Dict:
        ...
        try:
            ...
            self.cursor.execute('SELECT COUNT(*) FROM auth_users')
            stats["total"] = self.cursor.fetchone()[0]
            
            self.cursor.execute('SELECT COUNT(*) FROM auth_users WHERE is_active = 1')
            stats["active"] = self.cursor.fetchone()[0]
            
            self.cursor.execute('SELECT COUNT(*) FROM auth_users WHERE is_locked = 1')
            stats["locked"] = self.cursor.fetchone()[0]
            
            self.cursor.execute('SELECT role, COUNT(*) FROM auth_users GROUP BY role')
            for row in self.cursor.fetchall():
    ...
    def resolve_password_reset_ticket(self, ticket_id: int, new_password: str = None, 
        ...
        try:
            ...
            if approve:
                ...
                self.cursor.execute('''
                    UPDATE auth_users SET password_hash = ?, password_salt = ?, updated_at = ?
                    WHERE id = ?
...
if __name__ == "__main__":
    ...
    auth = AuthenticationManager(db_file=test_db)
