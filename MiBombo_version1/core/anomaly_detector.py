#!/usr/bin/env python3
"""
MiBombo Suite V1 - Module de Detection d'Anomalies Avance
============================================================
...
class AnomalyDetector:
    ...
    SENSITIVE_OIDS = {
        ...
        # Critical for SET
        "1.3.6.1.6.3.15.1.2.2.1.3": "snmpEnableAuthenTraps",
    ...
    def __init__(self, config: Dict = None):
        ...
        self.thresholds = {
            # Traffic Anomalies
            "unusual_ports": {161, 162, 10161, 10162},
            "max_packet_size": 1500, # MTU-ish, SNMP usually small
            
            # Security Constants
            "replay_window": 150, # RFC 3414 default (plus 150s drift)
        ...
        self.window_packets: deque = deque(maxlen=1000)
        ...
        # Persistence
        self._stats_file = "data/stats.json"
        self._load_stats()
    
    def _save_stats(self):
        """Sauvegarde les statistiques cumulées"""
        try:
            with self._lock:
                data = {
    ...
    def _load_stats(self):
        ...
        try:
            if os.path.exists(self._stats_file):
                ...
                with self._lock:
                    self.stats["total_packets_analyzed"] = data.get("total_packets_analyzed", 0)
                    self.stats["total_alerts_generated"] = data.get("total_alerts_generated", 0)
                    
                    # Fusionner les compteurs
                    for k, v in data.get("alerts_by_type", {}).items():
                        self.stats["alerts_by_type"][k] = v
                    
                    for k, v in data.get("alerts_by_severity", {}).items():
                        self.stats["alerts_by_severity"][k] = v
                
                self.logger.info(f"Stats restaurées: {self.stats['total_packets_analyzed']} paquets")
        except Exception as e:
    ...
    def _create_alert(self, anomaly_type: AnomalyType, severity: Severity,
        ...
        # === ENVOI TELEGRAM ===
        self._send_telegram_alert(alert, anomaly_type, severity, source_ip, details)
        
        return alert
    
    def _send_telegram_alert(self, alert: AnomalyAlert, anomaly_type: AnomalyType, 
                             severity: Severity, source_ip: str, details: Dict):
        """Envoie l'alerte via Telegram si configuré"""
        ...
        try:
            ...
            # Mapper les types d'anomalies vers les méthodes Telegram
            if anomaly_type == AnomalyType.FLOOD:
                pkt_count = details.get("packet_count", 0) if details else 0
                threshold = details.get("threshold", 100) if details else 100
                notifier.alert_flood(source_ip, pkt_count, threshold)
            
            elif anomaly_type == AnomalyType.NETWORK_SCAN:
                dest_count = details.get("destinations", 0) if details else 0
                notifier.alert_scan(source_ip, dest_count)
            
            elif anomaly_type == AnomalyType.TRAP_STORM:
                trap_count = details.get("trap_count", 0) if details else 0
                notifier.send_alert(notifier.Alert(
                    "⛈️ Trap Storm", alert.message,
                    notifier.AlertSeverity.CRITICAL, source_ip, details or {}
                ) if hasattr(notifier, 'Alert') else None)
            
            elif anomaly_type == AnomalyType.COMMUNITY_ENUM:
                community = details.get("community", "?") if details else "?"
                notifier.alert_suspect_community(source_ip, community)
            
            else:
                # Alerte générique pour les autres types
                from .telegram_notifier import Alert, AlertSeverity as TeleSeverity
                tele_severity = {
                    Severity.INFO: TeleSeverity.INFO,
                    Severity.WARNING: TeleSeverity.WARNING,
                    Severity.CRITICAL: TeleSeverity.CRITICAL,
                    Severity.EMERGENCY: TeleSeverity.CRITICAL,
                }.get(severity, TeleSeverity.WARNING)
            ...
            self.logger.debug(f"Telegram alert error: {e}")
    ...
    def analyze_packet(self, packet_data: Dict) -> List[AnomalyAlert]:
        ...
        # New Fields
        mac_src = packet_data.get("mac_src")
        packet_len = packet_data.get("packet_len", 0)
        ...
        # 8. Nouvelles detections (Port, Size, Critical SET, Spoofing)
        alerts.extend(self._detect_unusual_port(profile, ip_src, ip_dst, port_src, port_dst))
        alerts.extend(self._detect_large_packet(profile, ip_src, ip_dst, packet_len))
        alerts.extend(self._detect_critical_set(profile, ip_src, ip_dst, oids, pdu_type))
        
        if mac_src:
             alerts.extend(self._detect_spoofing(profile, ip_src, mac_src))

        # 9. Verifier le score de reputation
    ...
    def _detect_unusual_port(self, profile: IPProfile, ip_src: str, ip_dst: str, 
                             port_src: int, port_dst: int) -> List[AnomalyAlert]:
        """Detecte l'utilisation de ports non standards"""
        alerts = []
        valid_ports = self.thresholds["unusual_ports"]
        
        # Si aucun des ports n'est standard (ni src ni dst), c'est suspect
        if port_dst not in valid_ports and port_src not in valid_ports:
             alert = self._create_alert(
                AnomalyType.UNUSUAL_PORT,
                Severity.WARNING,
                ip_src, ip_dst,
                f"Trafic SNMP sur port inhabituel: Src={port_src} Dst={port_dst}",
                {"port_src": port_src, "port_dst": port_dst}
             )
             alerts.append(alert)
        return alerts

    def _detect_large_packet(self, profile: IPProfile, ip_src: str, ip_dst: str, 
                             packet_len: int) -> List[AnomalyAlert]:
        """Detecte les paquets anormalement volumineux"""
        alerts = []
        limit = self.thresholds["max_packet_size"]
        if packet_len > limit:
             alert = self._create_alert(
                AnomalyType.LARGE_RESPONSE,
                Severity.WARNING,
                ip_src, ip_dst,
                f"Paquet SNMP anormalement volumineux: {packet_len} octets (Limit {limit})",
                {"packet_len": packet_len, "limit": limit}
             )
             alerts.append(alert)
        return alerts

    def _detect_critical_set(self, profile: IPProfile, ip_src: str, ip_dst: str, 
                             oids: list, pdu_type: str) -> List[AnomalyAlert]:
        """Detecte les modifications (SET) d'OIDs critiques"""
        alerts = []
        if "set" not in pdu_type.lower():
            return alerts
            
        for oid_entry in oids:
            if isinstance(oid_entry, dict):
                oid = oid_entry.get("oid", "")
                if oid in self.SENSITIVE_OIDS:
                     alert = self._create_alert(
                        AnomalyType.SUSPICIOUS_OID,
                        Severity.CRITICAL,
                        ip_src, ip_dst,
                        f"Tentative de modification CRITIQUE (SET): {self.SENSITIVE_OIDS[oid]}",
                        {"oid": oid, "oid_name": self.SENSITIVE_OIDS[oid]}
                     )
                     alerts.append(alert)
                     profile.update_reputation(-20)
        return alerts

    def _detect_spoofing(self, profile: IPProfile, ip_src: str, mac_src: str) -> List[AnomalyAlert]:
        """Detecte l'usurpation d'IP via changement de MAC"""
        alerts = []
        if not mac_src or mac_src == "00:00:00:00:00:00":
            return alerts
            
        if profile.mac_address and profile.mac_address != mac_src:
             alert = self._create_alert(
                AnomalyType.BRUTE_FORCE, # Utilise BRUTE_FORCE ou autre car SPOOFING pas dans Enum
                Severity.EMERGENCY,
                ip_src, "local",
                f"ALERTE SPOOFING: IP {ip_src} a changé de MAC ! ({profile.mac_address} -> {mac_src})",
                {"old_mac": profile.mac_address, "new_mac": mac_src}
             )
             alerts.append(alert)
             profile.is_blacklisted = True
        elif not profile.mac_address:
             profile.mac_address = mac_src
    ...
    def clear_alerts(self):
        ...
        with self._lock:
            self.alerts.clear()
            self.stats["total_alerts_generated"] = 0
            
    def reset(self):
        """Réinitialise complètement le détecteur"""
        with self._lock:
            self.clear_alerts()
            ...
            self._save_stats()
            # logger.info("Détecteur d'anomalies réinitialisé")

    def save_stats(self):
        """Force la sauvegarde des statistiques"""
        self._save_stats()
