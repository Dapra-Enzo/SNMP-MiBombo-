...
class CaptureManager:
    """
    Gestionnaire singleton orchestrant la capture, l'analyse et le stockage des paquets SNMP.
    
    Utilité :
    - Initialiser et contrôler le Sniffer réseau et l'Analyseur de paquets.
    - Centraliser les données (paquets, alertes, stats) pour l'API.
    - Gérer la persistance en base de données et les fichiers PCAP.

    Fonctions associées :
    - start(iface, filt) : Démarre le thread de capture et la boucle d'analyse.
    - stop() : Arrête proprement la capture et ferme les ressources.
    - get_stats() : Renvoie les statistiques globales (paquets, débit, anomalies).
    - get_packets(limit) : Récupère les derniers paquets capturés pour l'affichage.
    """
    _instance = None
...
def create_app(enable_auth=True):
    ...
    def docs():
        endpoints = [
            ...
            # Authentication de base pour les endpoints
            ...
            # Utilisateurs CRUD avec authentification
            ...
            # SNMPv3 Users CRUD avec authentification
            ...
            # Packets avec authentification
            {"method": "GET", "url": "/api/packets", "desc": "Liste des paquets capturés"},
            
            # Alerts CRUD avec authentification
            ...
            # Devices CRUD avec authentification
