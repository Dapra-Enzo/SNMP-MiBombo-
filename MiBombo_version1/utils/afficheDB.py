#!/usr/bin/env python3
"""
Ce fichier afficheDB.py permet d'afficher le contenu de la base de données dans un fichier .db
pour cela il faut d'abord avoir une base de données .db
Ensuite il faut avoir le module cryptography installé
et enfin il faut avoir la variable SNIFFER_KEY definie
"""
...
def get_cipher():
    """Recupère a l'aide de la variable SNIFFER_KEY la clé de chiffrement"""
...
def decrypt_val(cipher, value):
    """Decrypte une valeur chiffrée avec cypher.decrypt"""
    if value is None:
        return None
    try:
...
def afficher_contenu_db(nom_db, mode_decrypt=False, table_filter=None, limit=50):
    """Affiche le Contenu de la BDD avec possibilité de decrypter les colonnes chiffrées"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Affiche une base SQLite MiBombo.")
