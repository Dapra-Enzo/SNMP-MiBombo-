...
class ProfilePanel(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        ctk.CTkLabel(self._info_frame, text="Informations",
                    ...
                    text_color=THEME["accent"]).pack(anchor="w", padx=20, pady=(15, 12))
        
        self._info_labels = {}
        for key, label in [("username", "Identifiant"), ("full_name", "Nom complet"),
                           ("email", "Email"), ("role", "Role"), ("last_login", "Derniere connexion")]:
        ...
        ctk.CTkFrame(self._info_frame, fg_color="transparent", height=15).pack()
        ...
        ctk.CTkLabel(pwd_frame, text="Changer le mot de passe",
                    font=ctk.CTkFont(size=15, weight="bold"),
                    text_color=THEME["accent"]).pack(anchor="w", padx=20, pady=(15, 15))
        
        # Ancien mot de passe
        ctk.CTkLabel(pwd_frame, text="Ancien mot de passe",
                    font=ctk.CTkFont(size=12),
                    text_color=THEME["text_secondary"]).pack(anchor="w", padx=20)
        self.old_pwd_entry = ctk.CTkEntry(pwd_frame, show="*", height=40,
                                         font=ctk.CTkFont(size=13),
                                         fg_color=THEME["bg_input"])
        self.old_pwd_entry.pack(fill="x", padx=20, pady=(5, 12))
        
        # Nouveau mot de passe
        ctk.CTkLabel(pwd_frame, text="Nouveau mot de passe",
                    font=ctk.CTkFont(size=12),
                    text_color=THEME["text_secondary"]).pack(anchor="w", padx=20)
        self.new_pwd_entry = ctk.CTkEntry(pwd_frame, show="*", height=40,
                                         font=ctk.CTkFont(size=13),
                                         fg_color=THEME["bg_input"])
        self.new_pwd_entry.pack(fill="x", padx=20, pady=(5, 12))
        
        # Confirmer
        ctk.CTkLabel(pwd_frame, text="Confirmer le nouveau mot de passe",
                    font=ctk.CTkFont(size=12),
                    text_color=THEME["text_secondary"]).pack(anchor="w", padx=20)
        self.confirm_pwd_entry = ctk.CTkEntry(pwd_frame, show="*", height=40,
                                             font=ctk.CTkFont(size=13),
                                             fg_color=THEME["bg_input"])
        self.confirm_pwd_entry.pack(fill="x", padx=20, pady=(5, 12))
        
        self.pwd_status_label = ctk.CTkLabel(pwd_frame, text="",
                                            font=ctk.CTkFont(size=12))
        self.pwd_status_label.pack(pady=5)
        
        ctk.CTkButton(pwd_frame, text="Modifier le mot de passe",
                     command=self._change_password,
                     height=40,
                     font=ctk.CTkFont(size=13),
                     fg_color=THEME["accent"]).pack(padx=20, pady=(5, 20), fill="x")
        ...
        self._perms_list = ctk.CTkTextbox(perms_frame, height=120,
                                         fg_color=THEME["bg_input"],
                                         font=ctk.CTkFont(size=12))
        self._perms_list.pack(fill="x", padx=20, pady=(0, 20))
...
class UserManagementPanel(ctk.CTkFrame):
    """
    Panneau d'administration complet pour la gestion des utilisateurs et des permissions.
    
    Utilité :
    - Lister tous les comptes utilisateurs avec leur statut (Actif/Inactif).
    - Créer, modifier ou désactiver des utilisateurs.
    - Gérer les tickets de support (Demandes de réinitialisation de MDP).
    - Consulter les logs d'audit des actions récentes.

    Fonctions associées :
    - refresh() : Recharge les données (utilisateurs, stats, tickets, logs).
    - _show_create_dialog() : Ouvre la modale de création d'un nouvel utilisateur.
    - _switch_tab(tab) : Bascule entre la vue "Utilisateurs" et la vue "Tickets".
    - _toggle_user(user) : Active ou désactive un compte utilisateur existant.
    """
    
    def __init__(self, parent, auth_manager: AuthenticationManager = None, **kwargs):
        super().__init__(parent, fg_color=THEME["bg_card"], corner_radius=8, **kwargs)
    ...
    def _build_detail_for_user(self, user: Dict):
        ...
        # Titre
        ctk.CTkLabel(self._detail_content, text=user.get("username", "?"),
                    font=ctk.CTkFont(size=16, weight="bold"),
                    text_color=THEME["text_primary"]).pack(anchor="w", padx=10, pady=(10, 15))
        ...
        info = [
            ...
            ("Statut", "Actif" if user.get("is_active") else "Inactif"),
            ("Cree le", user.get("created_at", "-")[:10] if user.get("created_at") else "-"),
            ("Derniere connexion", user.get("last_login", "")[:16] if user.get("last_login") else "Jamais"),
        ]
        ...
        for label, value in info:
            ...
            if label == "Statut":
                value_color = THEME["success"] if user.get("is_active") else THEME["error"]
            
            ctk.CTkLabel(row, text=value,
                        font=ctk.CTkFont(size=12),
                        text_color=value_color).pack(side="left", fill="x", expand=True)
        
        # Séparateur
        ctk.CTkFrame(self._detail_content, fg_color=THEME["border"], height=1).pack(fill="x", pady=15, padx=10)
        
        # Actions
        ctk.CTkLabel(self._detail_content, text="Actions",
                    font=ctk.CTkFont(size=13, weight="bold"),
                    text_color=THEME["text_secondary"]).pack(anchor="w", padx=10, pady=(0, 10))
        ...
        ctk.CTkButton(actions, text="Modifier",
                     ...
                     font=ctk.CTkFont(size=12),
                     fg_color=THEME["info"],
                     command=lambda: self._show_edit_dialog(user)).pack(side="left", padx=(0, 8))
        
        ctk.CTkButton(actions, text="Reinit. MDP",
                     width=110, height=35,
                     font=ctk.CTkFont(size=12),
                     fg_color=THEME["warning"],
                     command=lambda: self._reset_password(user)).pack(side="left", padx=(0, 8))
        
        if user.get("username") != "admin":
            btn_text = "Reactiver" if not user.get("is_active") else "Desactiver"
...
class UserDialog(ctk.CTkToplevel):
    """
    Boîte de dialogue modale pour la saisie des informations utilisateur (Création/Édition).
    
    Utilité :
    - Saisir les données d'un compte (Identifiant, Mot de passe, Rôle, Email).
    - Valider les entrées (Champs requis, complexité MDP).
    - S'adapter dynamiquement au mode (Création vs Édition).

    Fonctions associées :
    - _build_ui() : Génère le formulaire adapté au mode (champs pré-remplis si édition).
    - _save() : Valide les saisies et appelle l'AuthenticationManager pour persister les changements.
    """
    
    def __init__(self, parent, auth: AuthenticationManager, mode: str = "create", user: Dict = None):
        super().__init__(parent)
    ...
    def _build_ui(self):
        ...
        # Titre
        ctk.CTkLabel(main_frame, text="Nouvel utilisateur" if self.mode == "create" else "Modifier utilisateur",
                    font=ctk.CTkFont(size=16, weight="bold"),
                    text_color=THEME["accent"]).pack(pady=(0, 15))
        
        # Username
        ctk.CTkLabel(main_frame, text="Identifiant *",
                    font=ctk.CTkFont(size=11),
                    text_color=THEME["text_secondary"]).pack(anchor="w")
        
        self.username_entry = ctk.CTkEntry(main_frame, height=35, fg_color=THEME["bg_input"],
                                          font=ctk.CTkFont(size=12))
        ...
        # Mot de passe
        pwd_label = "Mot de passe *" if self.mode == "create" else "Nouveau MDP (vide = conserver)"
        ctk.CTkLabel(main_frame, text=pwd_label,
                    font=ctk.CTkFont(size=11),
                    text_color=THEME["text_secondary"]).pack(anchor="w")
        
        self.password_entry = ctk.CTkEntry(main_frame, height=35, show="*", fg_color=THEME["bg_input"],
                                          font=ctk.CTkFont(size=12))
        self.password_entry.pack(fill="x", pady=(3, 10))
        
        # Nom complet
        ctk.CTkLabel(main_frame, text="Nom complet",
                    font=ctk.CTkFont(size=11),
                    text_color=THEME["text_secondary"]).pack(anchor="w")
        
        self.fullname_entry = ctk.CTkEntry(main_frame, height=35, fg_color=THEME["bg_input"],
                                          font=ctk.CTkFont(size=12))
        self.fullname_entry.pack(fill="x", pady=(3, 10))
        
        if self.mode == "edit" and self.user.get("full_name"):
            self.fullname_entry.insert(0, self.user["full_name"])
        
        # Email
        ctk.CTkLabel(main_frame, text="Email",
                    font=ctk.CTkFont(size=11),
                    text_color=THEME["text_secondary"]).pack(anchor="w")
        
        self.email_entry = ctk.CTkEntry(main_frame, height=35, fg_color=THEME["bg_input"],
                                       font=ctk.CTkFont(size=12))
        self.email_entry.pack(fill="x", pady=(3, 10))
        
        if self.mode == "edit" and self.user.get("email"):
            self.email_entry.insert(0, self.user["email"])
        
        # Rôle
        ctk.CTkLabel(main_frame, text="Role *",
                    font=ctk.CTkFont(size=11),
                    text_color=THEME["text_secondary"]).pack(anchor="w")
        
        self.role_var = ctk.StringVar(value=self.user.get("role", "viewer") if self.user else "viewer")
        self.role_menu = ctk.CTkComboBox(main_frame, 
                                        values=list(ROLES.keys()),
                                        variable=self.role_var,
                                        height=35,
                                        font=ctk.CTkFont(size=12),
                                        fg_color=THEME["bg_input"])
        self.role_menu.pack(fill="x", pady=(3, 10))
        
        # Message d'erreur
        self.error_label = ctk.CTkLabel(main_frame, text="",
                                       font=ctk.CTkFont(size=11),
                                       text_color=THEME["error"])
